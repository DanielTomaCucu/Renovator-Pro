...
| # | Decizie | Motivare |
|---|---|---|
| D1 | **Login pe `username`, nu pe email.** Coloană nouă `users.username` (UNIQUE NOT NULL), `email` devine NULLABLE și rămâne în schemă. ⚠️ **Parțial revizuit de D2** (2026-07-22): `email` redevine OBLIGATORIU la register (nu la login — login rămâne strict username+parolă), necesar pt. resetarea parolei. | Cerință explicită user („numele și parola, atât"). Emailul nu se șterge din schemă — zero pierdere de date, ușor de reactivat. |
| D2 | ⚠️ **REVIZUIT 2026-07-22, apoi 2026-07-22 (email real).** Flux de resetare parolă IMPLEMENTAT: `email` devine din nou obligatoriu la register (cerință explicită user, deviere de la D1 original), `POST /api/auth/forgot-password` generează un token opac (30 min, single-use) și trimite emailul de resetare prin **Resend** (`adapter/out/email/ResendPasswordResetEmailSender`), cu linkul `${APP_FRONTEND_URL}/reset-password?token=...`. Răspunsul e uniform (`204`, mereu) — NU mai confirmă dacă emailul există în sistem (aliniat la practica standard, spre deosebire de varianta inițială „mod dev"). Fără `RESEND_API_KEY` setat (ex. dev local fără cont Resend), linkul se scrie în logul backend-ului în loc să eșueze fluxul. `POST /api/auth/reset-password` schimbă parola și revocă TOATE refresh token-urile userului. Vezi `docs/api-contract.md` pt. shape-uri. | Cerere explicită user: „vreau ca userul să-și poată reseta parola" + „cum fac sa trimit emailuri de resetare a parolei". Resend ales pt. cost zero la volumul actual și zero verificare DNS necesară (expeditor `onboarding@resend.dev`). |
...
| D4 | ⚠️ **REVIZUIT 2026-07-22.** Multi-proiect per user IMPLEMENTAT: `project_members` permitea deja mai multe rânduri per user la nivel de schemă (PK compus `project_id`+`user_id`) — limitarea era doar aplicativă (`findByUserId` presupunea un singur rezultat). Acum userul poate fi membru al mai multor proiecte, cu UN proiect „activ" per SESIUNE (nu per user) — encodat implicit prin `refresh_tokens.project_id` (V11), nu prin JWT-ul de access (care rămâne neschimbat: doar `sub`+`username`). Comutarea (`POST /api/auth/switch-project`) rotește refresh token-ul spre alt proiect unde userul e deja membru; alăturarea la un proiect nou ca user EXISTENT (`POST /api/auth/join-project`) creează apartenența + comută automat. Vezi `docs/api-contract.md`. | Cerere explicită user: „vreau să pot intra în alte proiecte cu acel cod, nu doar la înregistrare". Alternativa „proiect activ pe user" (coloană pe `users`) ar fi însemnat că toate tab-urile/dispozitivele unui user împart forțat același proiect activ; legarea de refresh token (=sesiune) evită asta fără cost suplimentar de schemă. |
...
| D6 | **Partajarea proiectului = cod de invitație.** Fiecare proiect are un cod scurt generat server-side, vizibil doar pentru OWNER (pagina Setări), regenerabil. Cine se alătură devine **EDITOR** (poate CRUD camere/elemente; nu modifică proiectul/bugetul total, nu vede/regenerează codul, nu șterge membri — astea rămân OWNER, conform blueprint §5.2). ⚠️ **Revizuit de D4** (2026-07-22): codul nu mai funcționează DOAR la register — un user deja autentificat se poate alătura oricând, din „Setări → Proiectele mele" (`POST /api/auth/join-project`), fără cont nou. | Cerință user (2026-07-18): 2+ persoane pe același proiect; extins 2026-07-22 la „alăturare cu cont existent", odată ce multi-proiect (D4) a eliminat blocajul „un user = un proiect". |
...
| D8 | **Confirmare parolă** — câmp `confirmPassword` la register ȘI la reset-password, verificat STRICT client-side (nu ajunge în request-ul către backend). | Plasă de siguranță standard împotriva unei greșeli de tastare la parolă — nu e o regulă de business, deci nu are ce căuta în contractul API/backend. |
...
- ~~Resetare parolă / schimbare parolă din UI~~ — **IMPLEMENTAT 2026-07-22, vezi D2 revizuit** (email real prin Resend, nu doar mod dev).
- Roluri configurabile la invitație / VIEWER în UI (D7 — codul dă mereu EDITOR).
- ~~Alăturarea unui cont DEJA existent la alt proiect~~ — **IMPLEMENTAT 2026-07-22, vezi D4/D6 revizuite** (`POST /api/auth/join-project`).
- ~~Multi-proiect per user~~ — **IMPLEMENTAT 2026-07-22, vezi D4 revizuit.**
- **Rămâne în afara scope-ului:** verificare email (contul e utilizabil imediat, fără confirmare prin link), 2FA, OAuth social login, schimbare parolă din interiorul aplicației (doar via `/forgot-password`, nu un formular „schimbă parola" în Setări), roluri configurabile la invitație / VIEWER în UI (D7).
