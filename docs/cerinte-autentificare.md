...
| D2 | ⚠️ **REVIZUIT 2026-07-22.** Flux de resetare parolă IMPLEMENTAT (mod dev): `email` devine din nou obligatoriu la register (cerință explicită user, deviere de la D1 original), `POST /api/auth/forgot-password` generează un token opac (30 min, single-use) și îl expune DIRECT în răspuns — niciun serviciu de email configurat, deci linkul apare în UI (`/forgot-password`), nu pleacă pe email real. `POST /api/auth/reset-password` schimbă parola și revocă TOATE refresh token-urile userului. Vezi `docs/api-contract.md` pt. shape-uri. | Cerere explicită user: „vreau ca userul să-și poată reseta parola". Fără infra de email reală (Resend/SendGrid etc.), varianta „link afișat direct" e singura funcțională azi — documentată clar ca simplificare de dev, nu practică de producție (în special: NU se ascunde dacă emailul există, spre deosebire de recomandarea standard). |
...
- ~~Resetare parolă / schimbare parolă din UI~~ — **IMPLEMENTAT 2026-07-22, vezi D2 revizuit.**
