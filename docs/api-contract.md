...
| `/api/auth/forgot-password` | `POST` | `{ email }`, fără auth | `200 { resetToken }` — ⚠️ **mod dev**: tokenul pleacă direct în răspuns (niciun serviciu de email configurat); `404` dacă emailul nu există (deviere asumată de la practica standard „răspuns uniform", vezi `PasswordResetAccountNotFoundException`) |
