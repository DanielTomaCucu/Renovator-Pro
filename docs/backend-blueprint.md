...
| Lombok | **Adoptat** (decizie revizuită cu userul, Faza 3+) — `@Getter`/`@Setter`/`@NoArgsConstructor`/`@AllArgsConstructor` pe entitățile JPA (`adapter/out/persistence/entity`), `@RequiredArgsConstructor` pe clasele Spring cu injecție prin constructor (adaptere + use case-uri din `application/usecase`). **NU în `domain`** — records/enums nu au nevoie de el, iar domeniul rămâne fără nicio adnotare de framework (regulă de arhitectură explicită, păstrată chiar dacă Lombok n-ar lăsa urmă la runtime). `scope=provided`, deci zero footprint în artefactul rulat. | cerință user; reduce boilerplate-ul repetitiv din entități/adaptere |
...
- *Pași:* entități `@Entity` în `adapter/out/persistence/entity` (JSONB prin `@JdbcTypeCode(SqlTypes.JSON)` pentru structurile per-perete; getters/setters/constructori prin **Lombok** — vezi §1), mapper **MapStruct** entity↔domain (cod generat la compilare, nu reflecție runtime), adapterele implementează porturile.
